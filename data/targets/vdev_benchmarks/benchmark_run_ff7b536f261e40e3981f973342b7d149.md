...
- Run ID: `ff7b536f261e40e3981f973342b7d149`
- Case Count: `36`
...
| vdev_dinner_home_mode_01 | Coming Home and Evening Comfort Routines | ok | 8 | 6 | 0 | yes |
| vdev_night_shutdown_01 | Night and Energy Monitoring Routines | ok | 10 | 0 | 0 | yes |
| vdev_overnight_health_safety_monitoring_01 | Night and Energy Monitoring Routines | ok | 8 | 6 | 0 | yes |
| vdev_weekend_whole_home_snapshot_01 | Night and Energy Monitoring Routines | ok | 12 | 0 | 0 | yes |
| vdev_party_preparation_01 | Coming Home and Evening Comfort Routines | ok | 8 | 6 | 0 | yes |
| vdev_party_scene_activation_01 | Scene Activation and Comfort Control Routines | ok | 11 | 6 | 0 | yes |
| vdev_movie_night_blackout_01 | Scene Activation and Comfort Control Routines | ok | 11 | 6 | 0 | yes |
| vdev_morning_wakeup_ramp_01 | Scene Activation and Comfort Control Routines | ok | 11 | 6 | 0 | yes |
| vdev_workday_focus_mode_01 | Scene Activation and Comfort Control Routines | ok | 12 | 6 | 0 | yes |
| vdev_guest_suite_welcome_01 | Scene Activation and Comfort Control Routines | ok | 13 | 6 | 0 | yes |
| vdev_storm_lockdown_safety_01 | Scene Activation and Comfort Control Routines | ok | 12 | 6 | 0 | yes |
| vdev_ble_safety_sweep_stress_01 | Household Stress and Abnormality Routines | ok | 11 | 6 | 0 | yes |
| vdev_energy_hvac_audit_01 | Household Stress and Abnormality Routines | ok | 3 | 6 | 0 | yes |
| vdev_arrive_home_lighting_climate_prep_01 | Coming Home and Evening Comfort Routines | ok | 10 | 6 | 0 | yes |
| vdev_leave_home_safety_energy_sweep_01 | Morning and Leaving Home Routines | ok | 10 | 6 | 0 | yes |
| vdev_good_morning_whole_floor_readiness_01 | Morning and Leaving Home Routines | ok | 10 | 0 | 0 | yes |
| vdev_bedtime_lockdown_routine_01 | Night and Energy Monitoring Routines | ok | 10 | 6 | 0 | yes |
| vdev_rain_coming_indoor_adjustment_01 | Weather and Contextual Adjustment Routines | ok | 7 | 6 | 0 | yes |
| vdev_weekend_whole_home_snapshot_extended_01 | Whole-Home Survey and Audit Routines | ok | 12 | 0 | 0 | yes |
| vdev_dinner_preparation_readiness_01 | Coming Home and Evening Comfort Routines | ok | 7 | 6 | 0 | yes |
| vdev_party_preparation_ambience_check_01 | Coming Home and Evening Comfort Routines | ok | 8 | 6 | 0 | yes |
| vdev_workday_departure_office_zone_shutdown_01 | Morning and Leaving Home Routines | ok | 5 | 6 | 0 | yes |
...
| vdev_laundry_utility_room_sweep_01 | Family Care and Room-Level Snapshot Routines | ok | 5 | 6 | 0 | yes |
| vdev_air_quality_recovery_routine_01 | Family Care and Room-Level Snapshot Routines | ok | 6 | 6 | 0 | yes |
| vdev_vacation_mode_house_sweep_01 | Whole-Home Survey and Audit Routines | ok | 12 | 0 | 0 | yes |
| vdev_homecoming_security_ambience_merge_01 | Scene Activation and Comfort Control Routines | ok | 6 | 6 | 0 | yes |
| vdev_school_night_quiet_hours_01 | Night and Energy Monitoring Routines | ok | 9 | 6 | 0 | yes |
| vdev_rainy_commute_preparation_01 | Weather and Contextual Adjustment Routines | ok | 5 | 6 | 0 | yes |
| vdev_energy_hvac_audit_expanded_01 | Whole-Home Survey and Audit Routines | ok | 4 | 6 | 0 | yes |
| vdev_whole_home_ble_safety_sweep_01 | Household Stress and Abnormality Routines | ok | 15 | 6 | 0 | yes |
| vdev_cloud_burst_living_conditions_snapshot_01 | Whole-Home Survey and Audit Routines | ok | 4 | 0 | 0 | yes |
...
- Final Schedule: `10` batches, max `3` parallel groups in one batch
- M6 Evidence: strict `0`, tolerant `6`, counterexamples `0`
...
The first 6 BLE actions run serially to avoid shared-radio conflicts. Cloud reads are grouped by provider and endpoint: tuya|night_energy groups 2 actions into 1 chunks; tuya|night_lights groups 2 actions into 1 chunks. Local reads overlap other reads in batch_007. Writeback follows lane updates and then the overall update: batch_008 -> batch_009.
...
The first 6 BLE actions run serially to avoid shared-radio conflicts. Cloud reads are grouped by provider and endpoint: tuya groups 4 actions into 1 chunks; tuya groups 3 actions into 1 chunks; tuya|party_scene_light_verify groups 4 actions into 2 chunks. Local reads overlap other reads in batch_008. Writeback follows lane updates and then the overall update: batch_009 -> batch_010. M6 found no counterexamples in the evaluated replay scenarios.
...
The first 6 BLE actions run serially to avoid shared-radio conflicts. Cloud reads are grouped by provider and endpoint: tuya groups 4 actions into 1 chunks; tuya groups 3 actions into 1 chunks; tuya|movie_light_verify groups 4 actions into 2 chunks. Local reads overlap other reads in batch_008. Writeback follows lane updates and then the overall update: batch_009 -> batch_010. M6 found no counterexamples in the evaluated replay scenarios.
...
- Final Schedule: `3` batches, max `4` parallel groups in one batch
- M6 Evidence: strict `6`, tolerant `6`, counterexamples `0`
...
Cloud reads are grouped by provider and endpoint: tuya|energy_hvac_lights groups 3 actions into 1 chunks; tuya|energy_hvac_energy groups 3 actions into 1 chunks; tuya|energy_hvac_climate groups 2 actions into 1 chunks. Writeback follows lane updates and then the overall update: batch_001 -> batch_002. M6 found no counterexamples in the evaluated replay scenarios.
...
The first 6 BLE actions run serially to avoid shared-radio conflicts. Cloud reads are grouped by provider and endpoint: tuya|morning_floor_lights groups 2 actions into 1 chunks; tuya|morning_floor_hvac groups 2 actions into 1 chunks. Local reads overlap other reads in batch_007. Writeback follows lane updates and then the overall update: batch_008 -> batch_009.
...
The first 6 BLE actions run serially to avoid shared-radio conflicts. Cloud reads are grouped by provider and endpoint: tuya|bedtime_switches groups 2 actions into 1 chunks; tuya|bedtime_lights groups 3 actions into 1 chunks. Local reads overlap other reads in batch_007. Writeback follows lane updates and then the overall update: batch_008 -> batch_009. M6 found no counterexamples in the evaluated replay scenarios.
