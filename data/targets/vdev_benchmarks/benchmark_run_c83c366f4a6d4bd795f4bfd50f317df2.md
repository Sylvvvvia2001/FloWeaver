...
- Run ID: `c83c366f4a6d4bd795f4bfd50f317df2`
- Case Count: `5`
...
| vdev_vacation_departure_final_audit_01 | Profile Coverage - Arrival and Departure Routines | ok | 26 | 6 | 0 | yes |
| vdev_weekend_whole_home_snapshot_profile_01 | Profile Coverage - Monitoring and Audit Routines | ok | 18 | 6 | 0 | yes |
| vdev_holiday_vacation_mode_full_audit_01 | Profile Coverage - Realistic Stress Routines | ok | 28 | 6 | 0 | yes |
...
The first 7 BLE actions run serially to avoid shared-radio conflicts. Local reads overlap other reads in batch_007. Writeback follows lane updates and then the overall update: batch_009 -> batch_010. M6 found no counterexamples in the evaluated replay scenarios.
...
- Final Schedule: `26` batches, max `6` parallel groups in one batch
- M6 Evidence: strict `6`, tolerant `6`, counterexamples `0`
...
The first 9 BLE actions run serially to avoid shared-radio conflicts. Local reads overlap other reads in batch_009. Writeback follows lane updates and then the overall update: batch_024 -> batch_025. M6 found no counterexamples in the evaluated replay scenarios.
...
- Final Schedule: `18` batches, max `6` parallel groups in one batch
- M6 Evidence: strict `6`, tolerant `6`, counterexamples `0`
...
- Final Schedule: `28` batches, max `6` parallel groups in one batch
- M6 Evidence: strict `6`, tolerant `6`, counterexamples `0`
...
The first 13 BLE actions run serially to avoid shared-radio conflicts. Cloud reads are grouped by provider and endpoint: tuya groups 2 actions into 1 chunks. Local reads overlap other reads in batch_013. Writeback follows lane updates and then the overall update: batch_026 -> batch_027. M6 found no counterexamples in the evaluated replay scenarios.
