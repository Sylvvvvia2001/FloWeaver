...
from dsl.contracts import ExecutionPlan, OptimizationTarget
from dsl.io import dump_json


@dataclass
class VDevComponentArtifact:
    output_root: str
    custom_component_dir: str
    domain: str
    service_name: str
    manifest_path: str
    init_path: str
    ...
    plan_path: str
    target_path: str
    install_doc_path: str
    profile_artifact_path: str = ""
    certificate_artifact_path: str = ""


class VDevCustomComponentAssembler:
    ...
    def _domain_and_service(self, target: OptimizationTarget) -> tuple[str, str, str, str]:
        entrypoint = target.entrypoint if isinstance(target.entrypoint, dict) else {}
        service_cfg = entrypoint.get("service", {}) if isinstance(entrypoint.get("service", {}), dict) else {}

        meta = target.meta if isinstance(target.meta, dict) else {}
        candidate_vdev_id = (
            str(meta.get("vdev_id", ""))
            or str(target.objectives.get("vdev_id", ""))
            or str(target.entrypoint.get("vdev_id", ""))
        )
        vdev_id = self._slug(candidate_vdev_id)
        if not vdev_id:
            domains = target.source_scope.get("domains", []) if isinstance(target.source_scope, dict) else []
            fallback = str(domains[0]) if domains else "generated"
            vdev_id = self._slug(fallback)

        meta_name = str(meta.get("name", "FloWeaver VDev"))
        domain = f"vdev_{vdev_id}"

        service_name = self._slug(str(service_cfg.get("name", "run")))
        vdev_name = str(target.validation.get("vdev_name", target.objectives.get("name", meta_name)))
        return domain, service_name, vdev_name, vdev_id

    @staticmethod
    def _render_manifest(domain: str, vdev_name: str) -> Dict[str, Any]:
        return {
            "domain": domain,
            "name": vdev_name,
            "version": "0.1.0",
            "documentation": "https://example.invalid/floweaver/vdev",
            "requirements": [],
            "codeowners": [],
            "dependencies": [],
        }
    ...
    @staticmethod
    def _render_const_py(domain: str, service_name: str) -> str:
        return (
            f'DOMAIN = "{domain}"\n'
            f'SERVICE_RUN = "{service_name}"\n'
...
class PlanRunner:
    ...
    def _trace_export_path(self, service_data: dict[str, Any]) -> Path | None:
        ...
        filename = f"floweaver_vdev_{vdev_id}_trace.json"
...
'''
    ...
    @staticmethod
    def _render_entities_py(domain: str) -> str:
        return (
            'from __future__ import annotations\n\n'
            f'DOMAIN = "{domain}"\n'
        )
    ...
    @staticmethod
    def _render_services_yaml(service_name: str) -> str:
        return (
            f"{service_name}:\n"
            "  name: Run VDev\n"
            "  description: Execute the generated VDev execution plan.\n"
            "  fields:\n"
            "    payload:\n"
            "      name: Payload\n"
            "      description: Optional runtime parameters consumed by action templates.\n"
            "      required: false\n"
            "      selector:\n"
            "        object:\n"
            "    runtime_metrics:\n"
            "      name: Runtime metrics\n"
            "      description: Optional runtime metrics/guard flags injected into guard evaluation.\n"
            "      required: false\n"
            "      selector:\n"
            "        object:\n"
            "    export_trace:\n"
            "      name: Export trace\n"
            "      description: Export the run trace to .storage or a custom path after execution.\n"
            "      required: false\n"
            "      selector:\n"
            "        boolean:\n"
            "    trace_path:\n"
            "      name: Trace path\n"
            "      description: Optional explicit path for trace export.\n"
            "      required: false\n"
            "      selector:\n"
            "        text:\n"
        )
    ...
    @staticmethod
    def _render_translation_json(vdev_name: str, service_name: str) -> Dict[str, Any]:
        return {
            "title": vdev_name,
            "services": {
                service_name: {
                    "name": "Run VDev",
                    "description": "Execute generated VDev action plan",
                }
            },
        }

    @staticmethod
    def _render_install_md(output_root: Path, domain: str, service_name: str) -> str:
        return (
            ...
            f"3. Call service `{domain}.{service_name}` from Developer Tools -> Services.\n"
    ...
    def _copy_optional(self, src: str | Path | None, dst: Path) -> str:
        if not src:
            return ""
        src_path = Path(src)
    ...
    def assemble(
    ...
    ) -> VDevComponentArtifact:
        domain, service_name, vdev_name, vdev_id = self._domain_and_service(optimization_target)
        output_root = self.out_dir / f"vdev_{self._slug(vdev_id or domain)}"
        component_dir = output_root / "custom_components" / domain
        ...
        executor_path = component_dir / "executor.py"
        entities_path = component_dir / "entities.py"
        services_path = component_dir / "services.yaml"
        plan_path = component_dir / "plan.json"
        target_path = component_dir / "target.json"
        install_doc_path = output_root / "install.md"
        translation_path = component_dir / "translations" / "en.json"

        dump_json(manifest_path, self._render_manifest(domain, vdev_name))
        self._write_text(const_path, self._render_const_py(domain, service_name))
        self._write_text(init_path, self._render_init_py())
        self._write_text(executor_path, self._render_executor_py())
        self._write_text(entities_path, self._render_entities_py(domain))
        self._write_text(services_path, self._render_services_yaml(service_name))
        dump_json(plan_path, execution_plan)
        dump_json(target_path, optimization_target)
        dump_json(translation_path, self._render_translation_json(vdev_name, service_name))
        self._write_text(install_doc_path, self._render_install_md(output_root, domain, service_name))

        profile_artifact_path = self._copy_optional(profile_path, artifacts_dir / "ha_profile.json")
        certificate_artifact_path = self._copy_optional(certificate_path, artifacts_dir / "execution_certificate.json")

        return VDevComponentArtifact(
            output_root=str(output_root),
            custom_component_dir=str(component_dir),
            domain=domain,
            ...
            install_doc_path=str(install_doc_path),
            profile_artifact_path=profile_artifact_path,
            certificate_artifact_path=certificate_artifact_path,
        )

    def attach_certificate(self, artifact: VDevComponentArtifact, certificate_path: str | Path | None) -> str:
        if not certificate_path:
            return ""
        component_dir = Path(artifact.custom_component_dir)
        dst = component_dir / "artifacts" / "execution_certificate.json"
        copied = self._copy_optional(certificate_path, dst)
        artifact.certificate_artifact_path = copied
        return copied
