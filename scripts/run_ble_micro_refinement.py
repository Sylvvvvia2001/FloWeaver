...
DEFAULT_CONFIG_PATH = TARGET_ROOT / "ble_micro_refinement_defaults.json"
RUN_ROOT = REPO_ROOT / "data" / "optimizer_runs" / "vdev_benchmarks"
OUTPUT_ROOT = REPO_ROOT / "data" / "optimizer_runs" / "vdev_benchmarks_ble_micro_refined"
REPORT_PATH = TARGET_ROOT / "BLE_MICRO_REFINEMENT_REPORT.md"
...
def _load_policy(path: Path) -> BLEMicroRefinementPolicy:
    return BLEMicroRefinementPolicy.from_dict(load_json(path))


def _load_case_result(case: Dict[str, Any], refiner: ConservativeBLEMicroRefiner) -> BLEMicroCaseResult:
...
def _write_case_output(result: BLEMicroCaseResult, case: Dict[str, Any], policy: BLEMicroRefinementPolicy) -> None:
    case_dir = OUTPUT_ROOT / result.vdev_id
    case_dir.mkdir(parents=True, exist_ok=True)
    dump_json(
        case_dir / "ble_micro_refinement.json",
        {
    ...
    dump_json(case_dir / "ble_micro_refined_plan_overlay.json", to_dict(result.refined_plan))


def _write_summary(results: List[BLEMicroCaseResult], cases: List[Dict[str, Any]], policy: BLEMicroRefinementPolicy) -> None:
    rows = []
    total_original = 0
    total_refined = 0
...
def _write_markdown(results: List[BLEMicroCaseResult], cases: List[Dict[str, Any]], policy: BLEMicroRefinementPolicy) -> None:
    lines: List[str] = []
    lines.append("# BLE Micro-Refinement Report")
    lines.append("")
    lines.append("This report applies post-processing to benchmark M5 plans.")
    lines.append("The policy accounts for shared scanners, connectable controllers, connection slots, device and session identity, and subscriptions.")
    lines.append("")
    lines.append("## Policy")
    lines.append("")
    ...
    for case, result in zip(cases, results):
        ...
        lines.append(f"- Additional saving from BLE micro-refinement: `{result.added_savings_ms} ms` (`{result.added_savings_ratio * 100:.1f}%`)")
        ...
        for corridor in result.selected_corridors:
            lines.append(
                ...
                f"`{' -> '.join(corridor.corridor.action_ids)}`; "
            ...
            lines.append(
                f"  constraints: scanner=`{corridor.validation.scanner_constraints_ok}`, "
                f"connectable=`{corridor.validation.connectable_constraints_ok}`, "
                f"slots=`{corridor.validation.slot_constraints_ok}`, "
                f"types=`{corridor.validation.type_constraints_ok}`, "
                f"rollback=`{corridor.validation.rollback_constraints_ok}`"
            )
        lines.append("")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run conservative BLE micro-refinement over existing benchmark plans.")
    ...
    refiner = ConservativeBLEMicroRefiner(policy=policy)
